...
pub struct Notification {
    ...
    pub hint_image: Option<ImageSource>,
    pub summary: String,
    pub body: String,
...
fn load_image_from_data(data: &DBusStruct) -> Option<ImageSource> {
...
impl super::bus::OrgFreedesktopNotifications for Arc<dyn Proxy> {
    ...
    fn notify(
        ...
        let app_icon = match app_icon.as_str() {
            "" => None,
            _ => Some(ImageSource::from(app_icon)),
        };
        ...
        let hint_image = if let Some(data) = prop_cast::<DBusStruct>(&hints, "image-data") {
            ...
            Some(ImageSource::from(path.as_str().unwrap()))
        } else {
            hints
                .get("image_path")
                .map(|path| ImageSource::from(path.as_str().unwrap()))
        };
        ...
        self.event(Event::new(
            Message::Notification(Box::new(notification)),
            None,
        ));
    ...
    fn close_notification(&mut self, id: u32) -> Result<(), dbus::MethodErr> {
        self.event(Event::new(
            Message::Close(id, None, CloseReason::Notified),
            None,
        ));
...
pub async fn task(proxy: impl Proxy) {
    ...
    proxy.event(Event::new(Message::Connected(connection), None));
